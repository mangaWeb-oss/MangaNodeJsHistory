...
const HISTORY_PATH = process.env.HISTORY_PATH || 'public/dataHistory.json'; // path in repo
...
module.exports = async (req, res) => {
  try {
    ...
    // add/replace item (ensure unique by id)
    const existing = (historyObj.history || []).filter(h => h.id !== item.id);
    existing.unshift(item); // newest first
    historyObj.history = existing;

    const updatedStr = JSON.stringify(historyObj, null, 2);
    const contentBase64 = Buffer.from(updatedStr, 'utf8').toString('base64');
    const commit = await putFile(contentBase64, `Update history: add ${item.id}`, sha);
